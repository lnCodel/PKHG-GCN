...
class PKHG(torch.nn.Module):
    def __init__(self, input_dim, num_classes, dropout, edgenet_input_dim, edge_dropout, hgc, lg,lg1,gl=0.5, K1=4):
        ...
        self.lg1 = lg1
        self.gconv_LR = nn.ModuleList()#Bilateral-GCN
        for i in range(lg1):
        ...
        self.edge_net = EL(input_dim=edgenet_input_dim, dropout=dropout)
        self.model_init()
        self.BD1 = PKFE()
    def model_init(self):
    ...
    def Select(self, edge_weight, mask, val, threshold):
    ...
    def step(self,index, weight):
        ...
        A = A + A.T
        A = A.float()
        val_1 = torch.sum(A, dim=1)
        for i in range(2, self.K):
            B = torch.matrix_power(A, i)
            all1 = torch.sum(B)
            B = B / all1
            val_k = torch.sum(B, dim=1)
            val_1 += val_k + val_1
        return val_1
    def forward(self, features_dis,features_hea, edge_index, edgenet_input, mask, enforce_edropout=False):
        ...
        logit = self.cls(torch.cat((chayi, stroke), dim=1))
        return logit,merged_tensor, 1, val

class PKFE(torch.nn.Module):
    def __init__(self,):
        super(PKFE, self).__init__() #X_stroke = a * X_left + b * X_right  求a, b
